...
def training_window():
    ...
    resultFrame = Frame(training_window, highlightbackground="black", highlightthickness=2, bg="#f7f3f2", width=1600, height=650)
    resultFrame.place(x=220, y=150, width=1600, height=650)
    ...
    # r_scroll_bar = Scrollbar(resultFrame)
    # r_scroll_bar.pack(side=RIGHT, fill=Y)
    # # 底部滚动条
    # b_scroll_bar = Scrollbar(resultFrame, orient=HORIZONTAL)
    # b_scroll_bar.pack(side=BOTTOM, fill=X)
    #
    # # text_view = tk.Text(resultFrame, width=200, height=200)
    # # result_text_view.config(yscrollcommand=r_scroll_bar.set, xscrollcommand=b_scroll_bar.set, font=f1)  # 绑定
    # # text_view.pack()
    #
    # r_scroll_bar.config(command=result_text_view.yview)  # 绑定
    # b_scroll_bar.config(command=result_text_view.xview)  # 绑定
    ...
    def create_options_view():
        ...
        # if select_model() in ['TransE', 'TransD', 'TransH']:
            # sele.state = 'normal'
        ...
        # [{"title": "Models", "value": "TransE"},
        #  {"title": "Bern", "value": 1},
        #  {"title": "Learning rate", "value": 0.1},
        #  {"title": "Number batch", "value": 512},
        #  {"title": "Loss", "value": "pair"},
        #  {"title": "Negative sampling", "value": "Bern"},
        #  {"title": "Number of negative sample", "value": 1},
        #  {"title": "Margin", "value": 0},
        #  {"title": "Lamb", "value": 0.1}
        #  ]
        ...
        #     labelW = 17
        #     boxW = 17
        #     # mb = tk.Menubutton(resultFrame, text='Training Options', relief='raised', font=f1, width=201)
        #     # # mb.pack(anchor=tk.NW)
        #     # mb.place(x=0, y=0, width=1597, height=40)
        #
        #     r_scroll_bar = Scrollbar(resultFrame)
        ...
        #
        #     text_view = tk.Text(resultFrame, width=200, height=200)
        #     text_view.config(yscrollcommand=r_scroll_bar.set, xscrollcommand=b_scroll_bar.set, font=f1)  # 绑定
        #     text_view.pack()
        #
        #     style_value = ttk.Style()
        #     style_value.configure("Text", rowheight=60, font=f1)
        ...
        #     b_scroll_bar.config(command=result_text_view.xview)  # 绑定
        #
        #     frame2 = tk.Frame(resultFrame, borderwidth=2, relief="groove")
        #     frames.append(frame2)
        #
        #     frame = tk.Frame(resultFrame, borderwidth=2, relief='groove')
        #     frames.append(frame)
        #     frame1 = tk.Frame(resultFrame, borderwidth=2, relief="groove")
        ...
        #         print('value的值:{}'.format(selected_Model.get()))
        #         if select_model() not in ['TransE', 'TransD', 'TransH']:
        #             selectLamb = ttk.Combobox(frame, textvariable=selected_lamb, values=select_lamb, width=100,
        #                                       font=f1, state='normal')
        #             selectLamb.place(x=270, y=200, width=230, height=30)
        #             selectMargin = ttk.Combobox(frame, textvariable=selected_margin, values=select_margin,
        #                                         width=100,
        #                                         font=f1, state='disable')
        #             selectMargin.place(x=20, y=200, width=230, height=30)
        #         else:
        ...
        #         return selected_Model.get()
        #
        #     selectModelLabel = Label(frame, text='KGE Model', font=f1, bg="#f7f3f2", anchor='w', width=labelW)
        #     # selectModelLabel.place(x=18, y=43, width=230, height=30)
        ...
        #     selected_Model.set("TransE")
        #     # selected_Model.trace('w', select_model)
        #     selectModel = ttk.Combobox(frame1, textvariable=selected_Model, values=select_Model, width=boxW,
        #                                font=f1)
        #     selectModel.bind('<<ComboboxSelected>>', choose)
        #     # widgets.append(selectModel)
        #     selectModel.pack(side='left')
        #     # selectModel.place(x=20, y=70, width=230, height=30)
        #     # selectModel.pack(padx=20, ipady=10)
        #
        #     selectMarginLabel = Label(frame, text='Select Margin', font=f1, bg="#f7f3f2", anchor='w', width=labelW)
        #     # selectMarginLabel.place(x=18, y=173, width=230, height=30)
        #     selectMarginLabel.pack(side='left')
        #
        #     def margin():
        #         margin_result = selected_margin.get()
        #         return selected_margin.get()
        ...
        #     selected_margin = StringVar(training_window)
        #     selected_margin.set("0")
        #     selectMargin = ttk.Combobox(frame1, textvariable=selected_margin, values=select_margin, width=boxW,
        #                                 font=f1, state='normal')
        #     # selectMargin.place(x=20, y=200, width=230, height=30)
        #     selectMargin.pack(side='left')
        #
        #     selectLambLabel = Label(frame, text='Select Lamb', font=f1, bg="#f7f3f2", anchor='w', width=labelW)
        #     # selectLambLabel.place(x=268, y=173, width=230, height=30)
        #     selectLambLabel.pack(side='left')
        #     #
        #     def lamb():
        #         lamb_result = selected_lamb.get()
        #         return selected_lamb.get()
        ...
        #     # if select_model() not in ['TransE', 'TransD', 'TransH']:
        #     # selected_lamb.state = 'normal'
        #     selectLamb = ttk.Combobox(frame1, textvariable=selected_lamb, values=select_lamb, width=boxW, font=f1,
        #                               state='disable')
        #     # selectLamb.place(x=270, y=200, width=230, height=30)
        ...
        #         return selected_Bernoulli.get()
        #
        #     BernoulliLabel = Label(frame, text='Select Bernoulli', font=f1, bg="#f7f3f2", anchor='w', width=labelW)
        #     # BernoulliLabel.place(x=18, y=108, width=230, height=30)
        #     BernoulliLabel.pack(side='left')
        #
        #     select_Bernoulli = [
        #         "True",
        #         "False"
        ...
        #     selected_Bernoulli.set("True")
        #     # selected_Bernoulli.trace('w', bernoulli)
        #     selectBernoulli = ttk.Combobox(frame1, textvariable=selected_Bernoulli, values=select_Bernoulli,
        #                                    width=boxW,
        #                                    font=f0)
        #     selectBernoulli.pack(side='left')
        #     # selectBernoulli.place(x=20, y=135, width=230, height=30)
        #     #
        #     def learning_rate():
        #         learning_rate_result = selected_learning_rate.get()
        #         return selected_learning_rate.get()
        #
        #     LearningRateLabel = Label(frame, text='Learning Rate', font=f1, bg="#f7f3f2", anchor='w', width=labelW)
        #     # LearningRateLabel.place(x=268, y=108, width=230, height=30)
        #     LearningRateLabel.pack(side='left')
        #
        #     select_learning_rate = [
        #         "0.1",
        #         "0.01",
        ...
        #     selected_learning_rate.set("0.1")
        #     # selected_learning_rate.trace('w', learning_rate)
        #     selectLearning_rate = ttk.Combobox(frame1, textvariable=selected_learning_rate,
        #                                        values=select_learning_rate, width=boxW,
        #                                        font=f0)
        #     # selectLearning_rate.place(x=270, y=135, width=260, height=30)
        #     selectLearning_rate.pack(side='left')
        #     #
        #     def batch_size():
        #         batch_size_result = selected_batch_size.get()
        #         return selected_batch_size.get()
        #
        #     BatchSizeLabel = Label(frame, text='Batch Size', font=f1, bg="#f7f3f2", anchor='w', width=labelW)
        #     # BatchSizeLabel.place(x=548, y=108, width=230, height=30)
        #     BatchSizeLabel.pack(side='left')
        #
        #     select_batch_size = [
        #         "512",
        #         "1024",
        ...
        #     selected_batch_size.set("512")
        #     # selected_batch_size.trace('w', batch_size)
        #     selectBatch_size = ttk.Combobox(frame1, textvariable=selected_batch_size, values=select_batch_size,
        #                                     width=boxW,
        #                                     font=f0)
        #     # selectBatch_size.place(x=550, y=135, width=260, height=30)
        #     selectBatch_size.pack(side='left')
        #     #
        #
        #     # -------------------------------------------------------
        #     def loss_function():
        ...
        #         return selected_loss_function.get()
        #
        #     LossFunctionLabel = Label(frame, text='Loss Function', font=f1, bg="#f7f3f2", anchor='w', width=labelW)
        #     # LossFunctionLabel.place(x=268, y=43, width=230, height=30)
        #     LossFunctionLabel.pack(side='left')
        #
        #     select_loss_function = [
        #         "pair",
        #         "point",
        ...
        #     selected_loss_function.set("pair")
        #     # selected_loss_function.trace('w', loss_function)
        #     selectLoss_function = ttk.Combobox(frame1, textvariable=selected_loss_function,
        #                                        values=select_loss_function, width=boxW,
        #                                        font=f0)
        #     # selectLoss_function.place(x=270, y=70, width=230, height=30)
        #     selectLoss_function.pack(side='left')
        #
        #     def negative_sampling():
        #         negative_sampling_result = selected_negative_sampling.get()
        #         return selected_negative_sampling.get()
        #
        #     NegativeSamplingLabel = Label(frame, text='Negative Sampling', font=f1, bg="#f7f3f2", anchor='w', width=labelW)
        #     # NegativeSamplingLabel.place(x=548, y=43, width=230, height=30)
        #     NegativeSamplingLabel.pack(side='left')
        #
        #     select_negative_sampling = [
        #         "Bernoulli",
        #     ]
        ...
        #     selected_negative_sampling.set("Bernoulli")
        #     # selected_negative_sampling.trace('w', negative_sampling)
        #     selectNegative_sampling = ttk.Combobox(frame1, textvariable=selected_negative_sampling,
        #                                            values=select_negative_sampling, width=boxW,
        #                                            font=f0)
        #     # selectNegative_sampling.place(x=550, y=70, width=260, height=30)
        #     selectNegative_sampling.pack(side='left')
        #     # -------------------------------------------------------
        #
        #     #
        #     def negative_sample_no():
        ...
        #         return selected_negative_sample_no.get()
        #
        #     NegativeSampleLabel = Label(frame, text='Negative Samples', font=f1, bg="#f7f3f2",
        #                                 anchor='w', width=labelW)
        #     # NegativeSampleLabel.place(x=548, y=173, width=290, height=30)
        #     NegativeSampleLabel.pack(side='left')
        #
        #     select_negative_sample_no = [
        #         "1",
        #         "50",
        ...
        #     selected_negative_sample_no.set('1')
        #     # selected_negative_sample_no.trace('w', negative_sample_no)
        #     selectNegative_sample_no = ttk.Combobox(frame1, textvariable=selected_negative_sample_no,
        #                                             values=select_negative_sample_no, width=boxW,
        #                                             font=f0)
        #     # selectNegative_sample_no.place(x=550, y=200, width=310, height=30)
        #     selectNegative_sample_no.pack(side='left')


        # def create_result_view_v2():
        #     training_content = ''
        #
        #     with open('results/ComplEx/dataset_unif_IS.txt', 'r', encoding='utf-8') as f:
        #         # content = f.read()
        #         # print(f.read())
        #         for line in f:
        #             training_content += line
        #
        #     result_text_view.insert(tk.INSERT, training_content)
        ...
        UploadButton = tk.Button(optionsFrame, text="Upload files", font=f1,
                                command=lambda: [training_window.destroy(), select_files_window_b()])
        ...
        # NewModelButton = tk.Button(optionsFrame, text="New Model", font=f1,
        #                         command=lambda: show_graph())
    ...
    #     b_scroll_bar.pack(side=BOTTOM, fill=X)
    #
    #     result_text_view = tk.Text(resultFrame, width=200, height=200)
    #     result_text_view.config(yscrollcommand=r_scroll_bar.set, xscrollcommand=b_scroll_bar.set, font=f1)  # 绑定
    #     result_text_view.pack()
    #
    #     style_value = ttk.Style()
    #     style_value.configure("Text", rowheight=60, font=f1)
    #
    ...
    #     # global result_text_view
    #     data = result_text_view.get(1.0, END)
    #     if len(data) > 1:
    #         result_text_view.delete(1.0, END)
    #     else:
    #         pass
    #     training_content = ''
    #
