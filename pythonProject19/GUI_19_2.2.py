...
def visualisation_window():
    ...
    def begin_search():
        ...
        network.draw_subgraph(graphFrame, entityBox.get(), relationBox.get(), max_hops, max_entities, file_entity, file_relation, file_fact)
    ...
    ent_checked = IntVar()
    num_entities = IntVar()
    Checkbutton(subgraphFrame, text="No. of Entities", variable=ent_checked, font=f1).pack(fill=X, pady=15, padx=30)
    ...
    hop_checked = IntVar()
    hop_count = IntVar()
    Checkbutton(subgraphFrame, text="Hop Count", variable=hop_checked, font=f1).pack(fill=X, pady=15, padx=30)
    ...
    # Search Button
    search_button = Button(subgraphFrame, text="Search", command=begin_search, font=f1)
