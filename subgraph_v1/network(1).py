...
def draw_sub(sub, index):
    ...
    plt.axis('off')
    save_path = 'image/pic-{}.png'.format(index + 1)
...
if __name__ == '__main__':
    # path of txt files
    entity = r'entity2id.txt'
    relate = r'relation2id.txt'
    train = r'train2id.txt'
    ...
    centers = get_muticenter(G, ['Toke', 'Harthacnut', 'England'], max_num)  # 3个中心节点
    print(len(centers))
    for i in range(len(centers)):
    ...
    txt = Text(root, width=180, height=300)
    ...
    for i in range(len(images)):
        # tk.Label(root, image=images[i], width=200, height=200).pack()
