...
class VisualFrame(tk.Frame):  # 继承Frame类
    ...
    def createPage(self):
        ...
        ent_checked = IntVar()
        num_entities = IntVar()
        Checkbutton(framec, text="No. of Entities", variable=ent_checked, font=140).pack(pady=15, padx=10)
        selectNumEntities = ttk.Combobox(framec, textvariable=num_entities, values=subgraphOptions, width=4,
                                         font=140)
        ...
        hop_checked = IntVar()
        hop_count = IntVar()
        Checkbutton(framec, text="Hop Count", width=18, variable=hop_checked, font=140).pack(pady=15, padx=10)
        selectHopCount = ttk.Combobox(framec, textvariable=hop_count, values=subgraphOptions, width=4, font=140)
        selectHopCount.pack()
...
class StatisticFrame(tk.Frame):  # 继承Frame类
    ...
    def __init__(self, root):
        ...
        self.frameg = None
        self.frameh = None
        self.train = r'dataset/train2id.txt'
    ...
    def show_category1(self):
        nnrelation.nn_categorization(self.train)
        global cate_text_view, framee, frameg, frameh, f1
...
class TrainFrame(tk.Frame):  # 继承Frame类
    def __init__(self, root):
        super().__init__(root)
        self.createPage()

    def createPage(self):
        tk.Button(self, text='Select Files').place(x=0, y=0, width=200, height=50)