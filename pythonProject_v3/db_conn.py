...
def upload_data(dbname, new_relation, new_entity, new_train):
    ...
    cursor.execute(sql)
    sql = 'CREATE TABLE relations (Relation_Name varchar(255), Relation_ID varchar(255));'
    cursor.execute(sql)
    ...
    for n in new_relation:
        if len(n) != 1:
            name = n[0]
            id = int(n[1])
            sql = 'INSERT INTO relations (Relation_Name, Relation_ID) values (%s, %s)'
    ...
    cursor.execute(sql)
    sql = 'CREATE TABLE entities (Entity_Name varchar(255), Entity_ID varchar(255));'
    cursor.execute(sql)
    ...
    for n in new_entity:
        if len(n) != 1:
            new_0 = n[0]
            new_1 = int(n[1])
            sql = 'INSERT INTO entities (Entity_Name, Entity_ID) values (%s, %s)'
    ...
    cursor.execute(sql)
    sql = 'CREATE TABLE facts (Data_1 varchar(255), Data_2 varchar(255), Data_3 varchar(255));'
    cursor.execute(sql)
    # insert train data
    for n in new_train:
        if len(n) != 1:
            ...
            new_1 = int(n[1])
            new_2 = int(n[2])
            sql = 'INSERT INTO facts(Data_1, Data_2, Data_3) values (%s, %s, %s)'
            #print(type(int(n[0])), type(int(n[1])), type(int(n[2])))
            #res = cursor.execute(sql, ((int(n[0])), (int(n[1])), (int(n[2]))))
